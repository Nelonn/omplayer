  ...
  void videoDecodeLoop() {
    while (!stop_requested_) {
      ...
      for (auto& frame : result.unwrap()) {
        ...
        if (y_src && u_src && v_src) {
          vf.rgba_plane.resize(static_cast<size_t>(pic.width) * pic.height * 4);
          uint8_t* dst = vf.rgba_plane.data();

          for (uint32_t y = 0; y < pic.height; ++y) {
            const uint8_t* y_ptr = y_src + y * vf.y_stride;
            const uint8_t* u_ptr = u_src + (y / 2) * vf.u_stride;
            const uint8_t* v_ptr = v_src + (y / 2) * vf.v_stride;
            uint8_t* d_ptr = dst + static_cast<size_t>(y) * pic.width * 4;
            ...
            for (uint32_t x = 0; x < pic.width; ++x) {
              ...
              d_ptr[x * 4 + 0] = static_cast<uint8_t>(r);
              d_ptr[x * 4 + 1] = static_cast<uint8_t>(g);
              d_ptr[x * 4 + 2] = static_cast<uint8_t>(b);
              d_ptr[x * 4 + 3] = 255;
            }
          }
        }
