#include "MultiChannelResampler.hpp"
...
MultiChannelResampler::MultiChannelResampler(int channels,
  ...
  for (int i = 0; i < channels; ++i) {
    resamplers_.emplace_back(std::make_unique<SincResampler>(io_sample_rate_ratio, SincResampler::ReadCB([this, i](float* destination, int frames) {
      ProvideInput(i, destination, frames);
    })));
  }
}
...
void MultiChannelResampler::ProvideInput(int channel, float* destination,
  ...
  if (channel == 0) {
    ...
    read_cb_(wrapped_resampler_audio_bus_.get());
  } else {
